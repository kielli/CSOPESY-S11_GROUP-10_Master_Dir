...
//     cout << "[Run Process] Running process: " << cpuProcess.getProcessName() << endl; // Debugging line
...
void CPU_Core::runProcess() {
    if (!cpuProcess.hasFinished()) {
        cout << "[Run Process] Running process: " << cpuProcess.getProcessName() << endl;
    ...
    isWorking = false;  // Mark core as idle after process completes
}
