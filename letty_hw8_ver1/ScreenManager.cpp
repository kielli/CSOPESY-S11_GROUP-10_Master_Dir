...
void ScreenManager::createScreen(const string& screenName) {
    ...
    if (screenMap.find(screenName) == screenMap.end()) {
        screens.push_back(make_unique<CreatedScreen>(screenName, this, ptrNumber, this->getMaxInstructions(), this->memPerProc));  // pass the ScreenManager pointer to CreatedScreen
...
void ScreenManager::createDummyScreen(const string& screenName, const int instructionCount) {
    ...
    if (screenMap.find(screenName) == screenMap.end()) {
        screens.push_back(make_unique<CreatedScreen>(screenName, this, ptrNumber, instructionCount, this->memPerProc));  // pass the ScreenManager pointer to CreatedScreen
        screenMap[screenName] = screens.size() - 1;  // map screen name to index
       /* if (this->scheduler == "\"fcfs\"")*/
           /* this->fscheduler.getProcessList().push_back(*screens.back());*/
        if (this->scheduler == "\"rr\"") {
