...
RR_Scheduler::RR_Scheduler(int cpuNum, int cpuCycle, int delayPerExec, int quantum, MemoryAllocator& memoryAllocator)
    : cpuNum(cpuNum), cpuCycle(cpuCycle), delayPerExec(delayPerExec), quantum(quantum), memoryAllocator(memoryAllocator) {
}
...
void RR_Scheduler::storeMemoryPtr(void* memory) {
    this->memory = memory;
}

void* RR_Scheduler::getMemoryPtr(void* memory) {
    return this->memory;
}
...
void RR_Scheduler::coreExecutionLoop(CPU& core) {
    ...
    while (!stopExecution) {
        ...
        if (!core.isCoreWorking() && !processList.empty()) {
           /* lock_guard<mutex> lock(processListMutex);*/
            if (!processList.empty()) {
                memory = memoryAllocator.allocate(processList.front().getRequiredMemory());

                if (memory != nullptr) {
                    cout << "Allocated memory for process: " << processList.front().getPName() << " ID:" << processList.front().getPID() << "\n";
                    cout << "Memory state: " << memoryAllocator.visualizeMemory() << "\n";
                    core.assignProcess(processList.front());
                    processList.erase(processList.begin());
                }
                else {
                    cout << "Insufficient memory for: " << processList.front().getPName() << " ID:" << processList.front().getPID() << "\n";
                }
            }
        ...
        if (core.isCoreWorking()) {
            ...
            while (!core.getCpuProcess().hasFinished()) {
                ...
                this_thread::sleep_for(chrono::seconds(delayPerExec / 1000));
            ...
            if (core.getCpuProcess().hasFinished()) {
                ...
                memoryAllocator.deallocate(memory); //deallocate memory
